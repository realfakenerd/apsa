<script lang="ts">
	...
	// Local
	let currentRailCategory: keyof typeof link | undefined = undefined;
	...
	// Reactive
	$: submenu = link[currentRailCategory ?? '/docs'];
	...
	<AppRail background="bg-transparent" border="border-r border-surface-500/30">
		...
		<AppRailTile bind:group={currentRailCategory} name="elements" value={'/elements'}>
			<svelte:fragment slot="lead"
				><Icon icon="tailwind" width="w-6" height="h-6" /></svelte:fragment>
			<span>Tailwind</span>
		</AppRailTile>
		<AppRailTile bind:group={currentRailCategory} name="svelte" value={'/svelte'}>
			<svelte:fragment slot="lead">
				<Icon icon="mdi:home" width="24px" class="h-5 w-5 fill-current" height="24px" />
			</svelte:fragment>
			<span>Svelte</span>
		</AppRailTile>
